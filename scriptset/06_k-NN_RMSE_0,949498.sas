...
close;
principal = inputData[, 1683:ncol(inputData)]; /*<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< change for different dataset */
...
use reco.svd;
read all var _all_ into principal;
close;
...
/* Initialisation */
nearestNeighbor = J(nrow(principal), k, 0); 		/* Matrix of nearest neighbors */
recommendation = J(nrow(rating), ncol(rating), 0);	/* Matrix with the estimated rating */
distance = J(nrow(principal), 1, 10**10); 			/* Vector of distances */

/* Loop - get the nearest neighbours */
do reference = 1 to nrow(principal);
	/* Measure distance, in this case Euclidean */
	do tested = 1 to nrow(principal);
		if reference ^= tested then do;
			distance[tested] = sum((principal[reference,]-principal[tested,])##2);
		end; else do;
