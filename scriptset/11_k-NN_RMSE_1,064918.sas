...
proc iml;
use reco.knn_all ;
read all into PredRating;
close; 

do replace = 1 to nrow(PredRating);

	if PredRating [replace,1] ^=. then PredRating [replace,1] = (PredRating [replace,1]*4)+1;
    if PredRating [replace,1]  =. then PredRating [replace,1] =&AvgRating  ;
end;

create reco.knn_all from PredRating;
append from PredRating;
close reco.knn_all;
quit;
...
/* Data to evaluate*/
%let PredRating = PredRatingBounded;		
%let tableName  = reco.knn_all_debiased   ; 
