...
/* Cut out trainting set */
proc sql;
create table reco.learnSet as
	select a.UserID
		,  a.ItemID
		,  a.rating
	from reco.sample a
...
proc fastclus 
	noprint
	data=reco.base_dense_normalized
	maxc=500
	maxiter=5
	DELETE=1
	out=reco.cluster
	outseed=reco.cluster_seed
	outstat=reco.cluster_stat;
    var _ALL_;
   run;

/* Remove useless columns – this table is used for k-NN & distance computation */
proc sql;
ALTER TABLE reco.cluster_seed
DROP COLUMN _CRIT_
DROP COLUMN CLUSTER
DROP COLUMN _FREQ_
DROP COLUMN _RMSSTD_
DROP COLUMN _RADIUS_
DROP COLUMN _NEAR_
DROP COLUMN _GAP_;
quit;

/* create clustinfo table: contains UserID and its cluster */
proc iml;
use reco.cluster;
read all var _num_ into inputData[colname=NumerNames];
close;

clustinfo = inputData[, ncol(inputData)-1];

create reco.clustinfo from clustinfo;
append from clustinfo;
close reco.clustinfo;
quit;
DATA reco.clustinfo;
SET reco.clustinfo;
UserID=_N_;    
...
Title3 "Number of Clusters";
/* Store & Print Number of Clusters */
proc sql;
select count(Col1)
into: nCluster
from reco.cluster_seed;
quit;
...
/* Set cluster recommendation to all users */
proc sql;
create table reco.result as select
	  a.UserID
	, b.ItemID
	, b.PredRating
	, a.Col1  as Cluster
 
	From reco.clustinfo a
	left join reco.knn_all b
	on a.Col1 = b.UserID;
quit;
...
data   reco.knn_all_debiased   /* (keep=UserID ItemID PredRating PredRatingBounded) */ /view=reco.knn_all_debiased;
	 merge reco.result         (keep=UserID ItemID PredRating in=a)
...
/* Sort: needed for datastep below (bottleneck) */
proc sort data=reco.result;
by UserId ItemId;
run;
